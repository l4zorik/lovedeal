...
import { View, Text, StyleSheet, Dimensions, TouchableOpacity, Image, Animated, Linking, Share } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import * as Haptics from 'expo-haptics';
import { COLORS, BORDER_RADIUS } from '../constants/theme';

const { width: SCREEN_WIDTH, height: SCREEN_HEIGHT } = Dimensions.get('window');
const FALLBACK_IMAGE = 'https://via.placeholder.com/800x1200/2A2118/FFF8F0?text=Reklama';
...
function AdCard({ deal, isLite, config, scrollY, onDismiss, customization }) {
  ...
  const handleCTA = useCallback(() => {
    if (config?.haptics) Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    if (deal.ctaUrl) Linking.openURL(deal.ctaUrl);
  }, [config, deal.ctaUrl]);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.imageArea} activeOpacity={1} onPress={handleDoubleTap}>
        <ImageComponent
      ...
      <View style={styles.adLabel}>
        <Ionicons name="megaphone" size={10} color={COLORS.adBadgeText} />
      ...
      <Animated.View style={[styles.rightBar, rightBarStyle]}>
        ...
        <TouchableOpacity style={styles.actionBtn} onPress={handleLike}>
          <Ionicons name={liked ? 'heart' : 'heart-outline'} size={30} color={liked ? COLORS.primary : COLORS.text} />
        ...
        <TouchableOpacity style={styles.actionBtn} onPress={handleSave}>
          <Ionicons name={saved ? 'bookmark' : 'bookmark-outline'} size={28} color={saved ? COLORS.accent : COLORS.text} />
        ...
        {config?.spinningDisc && (
          <Animated.View style={[styles.spinningDisc, { transform: [{ rotate: spin.interpolate({ inputRange: [0, 1], outputRange: ['0deg', '360deg'] }) }] }]}>
            <View style={styles.spinningDiscInner}>
              <Image source={{ uri: deal.avatar }} style={styles.spinningDiscImg} />
            </View>
          </Animated.View>
        )}
      </Animated.View>
      ...
      <Animated.View style={[styles.bottomInfo, bottomInfoStyle]}>
        <View style={styles.advertiserRow}>
          ...
          <Text style={styles.advertiserText}>{deal.advertiser}</Text>
        </View>

        <View style={styles.handleRow}>
          <Text style={styles.handle}>{deal.handle}</Text>
          {deal.verified && <Ionicons name="checkmark-circle" size={16} color={COLORS.accent} />}
        </View>
        ...
        <View style={styles.songRow}>
          ...
          <Text style={styles.songText} numberOfLines={1}>{deal.brand || ''}{deal.brand && deal.song ? ' · ' : ''}{deal.song || ''}</Text>
        </View>

        {(deal.discount || deal.salePrice) && (
          <View style={styles.discountRow}>
            {deal.discount ? (
              <View style={styles.discountBadge}>
                <Text style={styles.discountText}>-{deal.discount}{typeof deal.discount === 'number' ? '%' : ''}</Text>
              </View>
            ) : null}
            {deal.originalPrice ? <Text style={styles.originalPrice}>{deal.originalPrice}</Text> : null}
            {deal.salePrice ? <Text style={styles.salePrice}>{deal.salePrice}</Text> : null}
          </View>
        )}

        <TouchableOpacity style={styles.ctaButton} onPress={handleCTA}>
          <Text style={styles.ctaText}>{deal.cta || 'Více info'}</Text>
